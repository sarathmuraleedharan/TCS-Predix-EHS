...
public abstract class DatagridServiceImpl {
	...
	private static Logger log = LoggerFactory
			.getLogger(DatagridServiceImpl.class);
	...
	public Response getResponse(String id, String appId, String authorization,
			String start_time, String end_time)  {

		DatagridResponse response = getDatagridResponseObject();
		List<BaseKpiDataGrid> kpiList = getDataSourceHandler().getWidgetData(
				id, start_time, end_time, authorization);
		for (BaseKpiDataGrid groupKpi : kpiList) {
			response.getTableData().add(
					this.getObjectMapper().convertValue(
							getKpiDataGrid(groupKpi), Map.class));
		}
